-- Penalty shootouts decide a drawn match but never form part of its goals.
alter table public.friendly_matches
    add column if not exists team_a_penalty_score integer,
    add column if not exists team_b_penalty_score integer;

alter table public.friendly_matches
    drop constraint if exists friendly_matches_penalty_shootout_check;

alter table public.friendly_matches
    add constraint friendly_matches_penalty_shootout_check check (
        (team_a_penalty_score is null and team_b_penalty_score is null) or
        (
            team_a_score = team_b_score and
            team_a_penalty_score is not null and
            team_b_penalty_score is not null and
            team_a_penalty_score >= 0 and
            team_b_penalty_score >= 0 and
    ...
    for v_goal in select value from jsonb_array_elements(coalesce(p_goals, '[]'::jsonb)) loop
        ...
        if not exists (
            ...
            where player.value ->> 'player_id' = v_goal ->> 'player_id'
              and (
                  (not v_is_own_goal and player.value ->> 'team' = v_goal ->> 'team') or
                  (v_is_own_goal and player.value ->> 'team' <> v_goal ->> 'team')
              )
        ) then
        ...
        if not exists (
            ...
            raise exception 'Goalkeeper does not match the goal';
        end if;
    end loop;
end;
$$;

create or replace function public.validate_penalty_shootout(
    p_team_a_score integer,
    p_team_b_score integer,
...
begin
    if p_team_a_penalty_score is null and p_team_b_penalty_score is null then
        return;
    end if;
    if p_team_a_penalty_score is null or p_team_b_penalty_score is null
       or p_team_a_score <> p_team_b_score
       or p_team_a_penalty_score < 0 or p_team_b_penalty_score < 0
       or p_team_a_penalty_score = p_team_b_penalty_score then
        raise exception 'A penalty shootout requires a drawn score and one winner';
    end if;
end;
$$;

drop function if exists public.create_friendly_match_acta(date, integer, integer, jsonb, jsonb);
create function public.create_friendly_match_acta(
    p_match_date date,
    p_team_a_score integer,
    p_team_b_score integer,
    p_team_a_penalty_score integer,
...
drop function if exists public.update_friendly_match_acta(uuid, date, integer, integer, jsonb, jsonb);
create function public.update_friendly_match_acta(
    p_match_id uuid,
    p_match_date date,
    p_team_a_score integer,
    p_team_b_score integer,
...
drop function if exists public.get_admin_friendly_matches();
create function public.get_admin_friendly_matches()
returns table (
    id uuid,
    played_on date,
...
begin
    ...
    return query
    select m.id, m.played_on, m.team_a_score, m.team_b_score, m.team_a_penalty_score, m.team_b_penalty_score
    from public.friendly_matches m
    order by m.played_on desc;
end;
$$;

drop function if exists public.get_friendly_match_acta(uuid);
create function public.get_friendly_match_acta(p_match_id uuid)
returns table (
    match_date date,
    team_a_score integer,
    team_b_score integer,
...
begin
    ...
    select
        ...
        coalesce((
            select jsonb_agg(jsonb_build_object('player_id', mp.player_id, 'team', mp.team, 'was_goalkeeper', mp.was_goalkeeper) order by mp.player_id)
